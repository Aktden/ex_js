...
if (filterMovie.value) {
    ...
    if (sessions.length === 0) {
      sessionList.innerHTML = "<p>На это время сенсов нету</p>";
      return;
    }
    ...
    sessions.forEach((session) => {
      ...
      card.innerHTML = `
        <div class="session-info">
          ...
          <div class="session-meta">${movie ? movie.title : `Фильм id: ${session.movie_id}`} · ${session.price} Р · ${session.seats} место</div>
        </div>
        <button>✕</button>
      `;
      const btn = card.querySelector("button");
deleteBtn.oncclick = function(){
  deleteSession(session.id);
}
      sessionList.append(card);
    });
  } catch (err) {
    sessionList.innerHTML = "<li style='color:Pink'> Ошибка</li>";
