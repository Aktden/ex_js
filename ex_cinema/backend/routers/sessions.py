...
@router.get("/")
def get_sessions(movie_id: int=None):
    if movie_id:
        return [x for x in db if x["movie_id"] == movie_id]
    return db
...
def add_session(body: SessionIn):
    ...
    if not body.time.strip():
        raise HTTPException(400, "Время укаать")
    if body.price <= 0:
        raise HTTPException(400, "Цена только +")
    if body.seats <= 0:
        raise HTTPException(400, "мест не может быть 0")
